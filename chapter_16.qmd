...
Now, to facilitate the exposition, we introduce a temporary definition. We shall say that a subcollection $\mathcal{J}$ of $\mathcal{X}$ is a *tower*`\index{tower}`{=latex} if 
...
Let us say that a set $C$ in $\mathcal{J}_{0}$ is *comparable*`\index{comparable}`{=latex} if it is comparable with every set in $\mathcal{J}_{0}$; this means that if $A \in \mathcal{J}_{0}$, then either $A \subset C$ or $C \subset A$. To say that $\mathcal{J}_{0}$ is a chain means that all the sets in $\mathcal{J}_{0}$ are comparable. Comparable sets surely exist: $\emptyset$ is one of them. In the next couple of paragraphs we concentrate our attention on an arbitrary but temporarily fixed comparable set $C$. 
