...
the set is called the *empty set*`\index{empty}`{=latex}. 
...
::: {#cnj-pairing}
## Axiom of pairing`\index{axiom of pairing}`{=latex}
...
the set is called the *pair*`\index{pair}`{=latex} (or, by way of emphatic comparison with a subsequent concept, the *unordered pair*`\index{unordered pair}`{=latex}) formed by $a$ and $b$. 
...
The relation between @eq-3-1 and @eq-3-2 typifies something that occurs quite frequently. All the remaining principles of set construction are pseudo-special cases of the axiom of specification in the sense in which @eq-3-1 is a pseudo-special case of @eq-3-2. They all assert the existence of a set specified by a certain condition; if it were known in advance that there exists a set containing all the specified elements, then the existence of a set containing just them would indeed follow as a special case of the axiom of specification. 
...
Are all the sets obtained in this way distinct from one another? 

:::

Before continuing our study of set theory, we pause for a moment to discuss a notational matter. It seems natural to denote the set $B$ described in @eq-3-1 by $\{x: S(x)\}$; in the special case that was there considered
