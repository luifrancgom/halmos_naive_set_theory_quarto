...
according as it is or is not important to emphasize the index set $I$. (By a "non-empty family"`\index{non-empty family}`{=latex} we mean a family whose domain $I$ is not empty.) It follows immediately from the definition of intersections that if $I \neq \emptyset$, then a necessary and sufficient condition that $x$ belong $\bigcap_{i}A_{i}$ is that $x$ belong to $A_{i}$ for all $i$. 
...
If both $\{ A_{i} \}$ and $\{ B_{i} \}$ are families of sets, then
