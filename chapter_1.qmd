...
A possible relation between sets, more elementary than belonging, is *equality*`\index{equality}`{=latex}. The equality of two sets $A$ and $B$ is universally denoted by the familiar symbol 
...
If $A$ and $B$ are sets and if every element of $A$ is an element of $B$, we say that $A$ is a *subset*`\index{subset}`{=latex} of $B$, or $B$ *includes*`\index{inclusion}`{=latex} $A$, and we write
...
The wording of the definition implies that each set must be considered to be included in itself ($A \subset A$); this fact is described by saying that set inclusion is *reflexive*`\index{reflexive}`{=latex}. (Note that; in the same sense of the word, equality also is reflexive.) If $A$ and $B$ are sets such that $A \subset B$ and $A \neq B$, the word *proper*`\index{proper}`{=latex} is used (proper subset, proper inclusion). If $A$, $B$, and $C$ are sets such that $A \subset B$ and $B \subset C$, then $A \subset C$; this fact is described by saying that set inclusion is *transitive*`\index{transitive}`{=latex}. (This property is also shared by equality.)
