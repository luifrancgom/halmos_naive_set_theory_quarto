...
## Axiom of extension`\index{axiom of extension}`{=latex}
...
If $A$ and $B$ are sets and if every element of $A$ is an element of $B$, we say that $A$ is a `\textit{subset}\index{subset}`{=latex} `<em>subset</em>`{=html} of $B$, or $B$ `\textit{includes}\index{inclusion}`{=latex} `<em>includes</em>`{=html} $A$, and we write
...
The wording of the definition implies that each set must be considered to be included in itself ($A \subset A$); this fact is described by saying that set inclusion is `\textit{reflexive}\index{reflexive}`{=latex} `<em>reflexive</em>`{=html}. (Note that; in the same sense of the word, equality also is reflexive.) If $A$ and $B$ are sets such that $A \subset B$ and $A \neq B$, the word `\textit{proper}\index{proper}`{=latex} `<em>proper</em>`{=html} is used (proper subset, proper inclusion). If $A$, $B$, and $C$ are sets such that $A \subset B$ and $B \subset C$, then $A \subset C$; this fact is described by saying that set inclusion is `\textit{transitive}\index{transitive}`{=latex} `<em>transitive</em>`{=html}. (This property is also shared by equality.)

If $A$ and $B$ are sets such that $A \subset B$ and $B \subset A$, then $A$ and $B$ have the same elements and therefore, by the axiom of extension, $A = B$. This fact is described by saying that set inclusion is `\textit{antisymmetric}\index{antisymmetric}`{=latex} `<em>antisymmetric</em>`{=html}. (In this respect set inclusion behaves differently from equality. Equality is `\textit{symmetric}\index{symmetric}`{=latex} `<em>symmetric</em>`{=html}, in the sense that if $A = B$, then necessarily $B = A$.) The axiom of extension can, in fact, be reformulated in these terms: if $A$ and $B$ are sets, then a necessary and sufficient condition that $A = B$ is that both $A \subset B$ and $B \subset A$. Correspondingly , almost all proofs of equalities between two sets $A$ and $B$ are split into two parts; first show that $A \subset B$, and then show that $B \subset A$.
