...
As the book title says, this the famous set theory book *Naive Set Theory* by Paul Richard Halmos, first published in 1960 by D. Van Nostrand Company, INC., part of a series called *The University Series in Undergraduate Mathematics*.
...
Even though the book was freely available online, there are three reasons for this project. First, the book in its digitized state is perfectly readable, but it doesn't allow searching words with {{< kbd mac=Command-F win=Ctrl-F linux=Ctrl-F >}} and doesn't have a interactable summary with it. The second is to update the book by correcting the errors in the original version, following the published [errata](https://en.wikipedia.org/wiki/Naive_Set_Theory_(book)#Errata) - as noticed, and updated in this edition, by Michał Zdunek. The third reason is purely personal, I have a passion and gratitude for this book and, while I want to learn OCR, I decided to re-edit it as a homage.
